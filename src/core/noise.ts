...
export const smoothNoise = (x: number, y: number, seed: number): number => {
  ...
  // Corners
  const bl = (hash(floorX, floorY, seed) % 1000) / 1000;
  const br = (hash(floorX + 1, floorY, seed) % 1000) / 1000;
  const tl = (hash(floorX, floorY + 1, seed) % 1000) / 1000;
  const tr = (hash(floorX + 1, floorY + 1, seed) % 1000) / 1000;
