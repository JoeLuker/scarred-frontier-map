...
import { WorldGenConfig } from '../types';
    ...
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="bg-slate-900 w-full max-w-md border border-slate-700 rounded-xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                    <div className="space-y-1">
                        <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-slate-400">
                            ...
                            <span className="text-indigo-400">{Math.round(config.waterLevel * 100)}%</span>
                        </div>
                        <input 
                            type="range" min="0" max="1" step="0.05" 
                            value={config.waterLevel}
                            onChange={(e) => handleSliderChange('waterLevel', e.target.value)}
                    ...
                    <div className="space-y-1">
                        <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-slate-400">
                            <span>Mountain Range Density</span>
                            <span className="text-indigo-400">{Math.round(config.mountainLevel * 100)}%</span>
                        </div>
                        <input 
                            type="range" min="0" max="1" step="0.05" 
                            value={config.mountainLevel}
                            onChange={(e) => handleSliderChange('mountainLevel', e.target.value)}
                            className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                        />
                    </div>
                    ...
                    <div className="space-y-1">
                        <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-slate-400">
                            ...
                            <span className="text-indigo-400">{Math.round(config.vegetationLevel * 100)}%</span>
                        </div>
                        <input 
                            type="range" min="0" max="1" step="0.05" 
                            value={config.vegetationLevel}
                            onChange={(e) => handleSliderChange('vegetationLevel', e.target.value)}
                            className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                        />
                    </div>
                    ...
                    <div className="space-y-1">
                        <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-slate-400">
                            ...
                            <span className="text-indigo-400">{Math.round(config.riverDensity * 100)}%</span>
                        </div>
                        <input 
                            type="range" min="0" max="1" step="0.05" 
                    ...
                    <div className="space-y-1">
                        <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-slate-400">
                            <span>World Ruggedness (Scale)</span>
                            <span className="text-indigo-400">{Math.round(config.ruggedness * 100)}%</span>
                        </div>
                        <input 
                            type="range" min="0" max="1" step="0.05" 
                            value={config.ruggedness}
                            onChange={(e) => handleSliderChange('ruggedness', e.target.value)}
                            className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                        />
                    </div>
                ...
                {/* Option to preserve explored areas */}
                <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700 flex items-center justify-between cursor-pointer hover:bg-slate-800 transition-colors"
