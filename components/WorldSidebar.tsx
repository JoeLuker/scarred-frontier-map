...
import { HexData, PartySpeed } from '../types';
import { Map as MapIcon, ChevronRight, Wand2, Download, Sun, Moon, Eye, Loader2, Layers, RotateCcw, RotateCw } from 'lucide-react';

interface WorldSidebarProps {
  hexes: HexData[];
  onFocusRegion: (groupId: string) => void;
  campaignTime: number;
  setCampaignTime: (val: number) => void;
  onOpenWizard: () => void;
  onOpenEditor: () => void;
  partySpeed: PartySpeed;
  setPartySpeed: (s: PartySpeed) => void;
  onRevealAll: () => void;
...
export const WorldSidebar: React.FC<WorldSidebarProps> = ({ 
  ...
  const days = Math.floor(campaignTime / 24) + 1;
  const hour = campaignTime % 24;
            ...
            <div className="w-64 bg-slate-950/95 backdrop-blur-xl border border-slate-800 shadow-2xl flex flex-col h-full rounded-2xl overflow-hidden animate-in slide-in-from-left-4 fade-in duration-300">
                ...
                <div className="p-4 border-b border-slate-800 bg-gradient-to-b from-slate-900/50 to-transparent">
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2 text-slate-400 bg-slate-900/80 px-2 py-1 rounded-md border border-slate-800">
                             {isNight ? <Moon size={12} className="text-indigo-400" /> : <Sun size={12} className="text-amber-400" />}
                             <span className="text-[10px] font-bold uppercase tracking-wider">Day {days}</span>
                        </div>
                    </div>
                    
                    <div className="text-3xl font-mono text-white mb-4 text-center tracking-widest font-light">
                        {timeString}
                    </div>
                    
                    <div className="grid grid-cols-2 gap-2">
                        <button 
                            onClick={() => setCampaignTime(campaignTime + 1)}
                            className="bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] font-bold py-2 rounded-lg border border-slate-700 transition-colors flex items-center justify-center gap-1"
                        >
                            +1 HR
                        </button>
                        <button 
                            onClick={() => setCampaignTime(campaignTime + 8)}
                            className="bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] font-bold py-2 rounded-lg border border-slate-700 transition-colors flex items-center justify-center gap-1"
                        >
                            REST (8h)
                        </button>
